...
import { UsaaMockApiService } from '@usaa/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root',
})
export class UsaaMockApiInterceptor implements HttpInterceptor {
    constructor(
        @Inject(USAA_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private usaaMockApiService: UsaaMockApiService
    ) {}

    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        // Try to get the request handler
        const { handler, urlParams } = this.usaaMockApiService.findHandler(
